...
graph TB
    subgraph Clients["Client Layer"]
        ...
        PWA["Mobile PWA"]
        VN["Voice Node XIAO ESP32S3 Sense"]
    end
...
#### Mode C — Ambient (Voice Node)

```
Voice node (XIAO ESP32S3) mic always listening for wake word
    → "hey computer" detected on-device (TFLite Micro, APP CPU)
    → Device plays a short audio chime (acknowledgement)
    → Device streams WAV audio to Voice Service via TLS WebSocket
    → Silence/timeout (VAD on APP CPU) → stream ends
    → Whisper transcribes on server
    → Sent to Onyx API as owner's message
    → Response generated by LLM
    → TTS converts response to audio on server (Piper)
    → WAV frames streamed back to device via WebSocket
    → Device plays response through MAX98357A amp + speaker
```
...
- Accept audio streams from the embedded voice node via the same WebSocket protocol
...
|---|---|---|
| `WS /voice/converse` | WebSocket | Browser, PWA, voice node — full-duplex voice chat (audio in, tokens + WAV out) |
...
**Planned (Phase 6 — voice node):**

The voice node uses the existing `WS /voice/converse` protocol — no new endpoint needed. The wire protocol already supports binary audio streaming and WAV response frames.

### 5.3 Embedded Voice Node

A custom Zephyr RTOS firmware device (XIAO ESP32S3 Sense) running two independent images (AMP):

- **APP CPU (Core 1):** runs TFLite Micro for always-on wake word detection; runs VAD (RMS-based silence detection)
- **PRO CPU (Core 0):** on wake event from APP CPU via IPM, opens TLS WebSocket to Voice Service, streams WAV audio, receives and plays back WAV response frames through a MAX98357A Class D amplifier
- Reconnects automatically if the WebSocket drops
- See [`embedded/voice_node/`](../embedded/voice_node/) for firmware and specs
...
| 6 | Embedded voice node deployed (XIAO ESP32S3 Sense) — uses existing `WS /voice/converse` | — |
